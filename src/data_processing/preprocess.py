...
def worker_q_transform_and_resize(args):
    ...
    Usamos PIL en lugar de TensorFlow para evitar crasheos de memoria en forked processes.
    """
    time_series_data, sample_rate = args
    try:
        ts = TimeSeries(time_series_data, sample_rate=sample_rate)
        # 1. Calculamos la transformada
        q_transform = ts.q_transform(frange=(20, 800), qrange=(8, 12))
        
        # 2. GWpy devuelve (Tiempo, Frecuencia). La transponemos a (Frecuencia, Tiempo)
        qt_value = q_transform.value.T 
        
        if RESIZE_ENABLED:
            # 3. Redimensionar usando PIL (Seguro para Multiprocessing)
            # mode='F' indica que es un array de floats de 32 bits
            img = Image.fromarray(qt_value)
            
            # PIL requiere el tamaño como (Ancho, Alto) -> (Tiempo, Frecuencia) -> (512, 256)
            img_resized = img.resize((TARGET_SHAPE[1], TARGET_SHAPE[0]), resample=Image.BILINEAR)
            
            resized_array = np.array(img_resized)
            return np.expand_dims(resized_array, axis=-1).astype(np.float32)
        else:
            return np.expand_dims(qt_value, axis=-1).astype(np.float32)

    except Exception as e:
        # AHORA SÍ: Imprimimos el error real para que no sea un asesino silencioso
        print(f"\n🚨 ERROR CRÍTICO EN WORKER: {e}")
        final_shape = TARGET_SHAPE if RESIZE_ENABLED else (800, 2000)
